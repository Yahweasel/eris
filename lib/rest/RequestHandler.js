    ...
    request(method, url, auth, body, file, _route, short) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const actualCall = () => {
                ...
                req.once("response", (resp) => {
                    ...
                    _respStream.on("data", (str) => {
                        ...
                        dbBucketUpdate.run(rateLimit);
                        dbRateLimitInit.run(rateLimit);
                        dbRateLimitUpdate.run(rateLimit);
            ...
            async function waitForRateLimit() {
                let bucketRow = dbBucketFetch.get({ROUTE: route});
                let bucket = bucketRow ? bucketRow.bucket : route;

                while (true) {
                    let now = Date.now();
                    dbRateLimitInit.run({BUCKET: bucket, RESET: now});
                    let rateLimitRow = dbRateLimitFetch.get({BUCKET: bucket});
                    ...
                    let res = dbRateLimitTake.run(next);
                    if (!res.changes) {
