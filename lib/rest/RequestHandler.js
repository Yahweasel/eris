...
const db = new SQLite("rate-limits.db");
...
const dbRateLimitFetch = db.prepare("SELECT * FROM rate_limits WHERE bucket=@BUCKET;");
const dbRateLimitInit = db.prepare("INSERT OR IGNORE INTO rate_limits (bucket, counter, remaining, reset) VALUES (@BUCKET, 0, 0, @RESET);");
const dbRateLimitUpdate = db.prepare("UPDATE rate_limits SET counter=counter+1, remaining=@REMAINING, reset=@RESET WHERE bucket=@BUCKET;");
const dbRateLimitTake = db.prepare(`
UPDATE rate_limits SET counter=@COUNTER+1, remaining=@REMAINING, reset=@RESET
    WHERE bucket=@BUCKET AND counter=@COUNTER;`);
const dbRateLimitDrop = db.prepare("DELETE FROM rate_limits WHERE bucket=@BUCKET;");
const dbBucketFetch = db.prepare("SELECT * FROM buckets WHERE route=@ROUTE;");
const dbBucketUpdate = db.prepare("INSERT OR REPLACE INTO buckets (route, bucket) VALUES (@ROUTE, @BUCKET);");
    ...
    request(method, url, auth, body, file, _route, short) {
        ...
        return new Promise((resolve, reject) => {
                ...
                req.once("response", (resp) => {
                    ...
                    _respStream.on("data", (str) => {
                        ...
                        if (rateLimit.BUCKET !== route)
                            dbRun(dbRateLimitDrop, {BUCKET: route});
                        ...
                        if(retryAfter >= 0) {
                            if(resp.headers["x-ratelimit-global"]) {
                                ...
                                dbRun(dbRateLimitInit, globalRL);
                                dbRun(dbRateLimitUpdate, globalRL);
                        ...
                        dbRun(dbBucketUpdate, rateLimit);
                        dbRun(dbRateLimitInit, rateLimit);
                        dbRun(dbRateLimitUpdate, rateLimit);
            ...
            async function waitForRateLimit() {
                let bucketRow = dbGet(dbBucketFetch, {ROUTE: route});
                ...
                while (true) {
                    ...
                    let globalLimit = dbGet(dbRateLimitFetch, {BUCKET: "global"});
                    ...
                    dbRun(dbRateLimitInit, {BUCKET: bucket, RESET: now});
                    let rateLimitRow = dbGet(dbRateLimitFetch, {BUCKET: bucket});
                    ...
                    let res = dbRun(dbRateLimitTake, next);
                    if (!res.changes) {
